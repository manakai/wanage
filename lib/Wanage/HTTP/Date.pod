...
=head1 SYNOPSIS
  ...
  my $time = parse_date $http->get_request_header ('If-Modified-Since');

=head1 DESCRIPTION

The C<Wanage::HTTP::Date> module exports a function, C<parse_date>,
which converts a byte string representing an HTTP C<date-time> into an
integer value representing a date-time in so-called Unix epoch
(time_t).

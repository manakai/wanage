...
C<$Wanage::App::AllowedURLSchemes>, where keys are normalized URL
...
C<$Wanage::App::AllowedHostname>.  By default, the regular expression
...
it is not specified, the C<$Wanage::App::AllowedMIMETypes> value is
...
is not specified, the C<$Wanage::App::AllowedRequestMethods> value is
used instead.  In either case, the list must be supplied as a hash

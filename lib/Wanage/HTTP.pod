...
If the C<$Wanage::HTTP::UseXForwardedScheme> variable is set to true,
the C<X-Forwarded-Scheme:> header, if any, is used to determine the
...
=item $http->set_response_plain_text ($text)

Set a plain text to the response-body, in UTF-8.  The C<Content-Type:>
header is set to C<text/plain; charset=utf-8>.

This is equivalent to invoking C<set_response_header>,
C<send_response_body_as_text>, and C<close_response_body> methods with
appropriate arguments.

=item $http->set_response_html ($text)

Set an HTML document to the response-body, in UTF-8.  The
C<Content-Type:> header is set to C<text/html; charset=utf-8>.
